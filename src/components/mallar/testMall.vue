  ...
  <div>
    <v-layout class="v-card grey lighten-5" pb-5 row wrap>
      ...
      <v-flex xs12 class="secondary--text" px-5 py-3>
        <div class="mb-4" :key="i" v-for="(x, i) in $store.state.mallData.work">
      ...
      <v-flex v-if="$store.state.mallData.education.length > 0" xs12>
        <v-layout row wrap>
          <div :class="{[$store.state.colorTheme]: true}">
            ...
            <v-flex style="padding:2px 2px" xs12 :class="{[$store.state.colorTheme]: true}"></v-flex>
            <v-flex xs12 class="secondary--text" px-5 py-3>
              <div class="pb-3" :key="i" v-for="(x, i) in $store.state.mallData.education">
                <div class="subheading bold secondary--text">
                  {{ x.subject }}
                </div>
      ...
      <v-flex v-if="$store.state.mallData.languages.length > 0" xs12>
        <v-layout row wrap>
          <div :class="{[$store.state.colorTheme]: true}">
            ...
            <v-flex style="padding:2px 2px" xs12 :class="{[$store.state.colorTheme]: true}"></v-flex>
            <v-flex xs12 class="secondary--text" px-5 py-3>
              <div class="pb-3" :key="i" v-for="(x, i) in $store.state.mallData.languages">
      ...
      <v-flex v-if="$store.state.mallData.referenser.length > 0" xs12>
        <v-layout row wrap>
          <div :class="{[$store.state.colorTheme]: true}">
            ...
            <v-flex style="padding:2px 2px" xs12 :class="{[$store.state.colorTheme]: true}"></v-flex>
            <v-flex xs12 class="secondary--text" px-5 py-3>
              <div style="width:50%" :key="i" v-for="(x, i) in $store.state.mallData.referenser" :class="{'grey lighten-3' : i % 2 === 0, 'white' : i % 2 !== 0, 'px-1 my-3 py-1 body-1' : true}">
                <div class="bold">
                  {{ x.name }}, {{ x.relation }}
                </div>
                  <div class="body-2">
