<template>
  <div>
    <v-layout class="v-card white white--text" row wrap>
      <v-flex :class="{[$store.state.colorTheme] : true}" xs4>
        <div class="display-1 px-3 py-3 bold white--text">
          {{ $store.state.mallData.name }}
        </div>
        <div :class="{[$store.state.colorTheme] : true, [headingClass] : true}">
          ...
          <div class="pb-5 pt-2">
            <div :key="x.id" v-for="x in $store.state.mallData.languages" class="py-1 px-3">
              ...
              <div style="position:relative">
                <v-progress-linear height="10px" :value="x.niva * 20" color="white"></v-progress-linear>
                <div style="position:absolute;right:0;top:12px" class="white--text">
                  {{ level(x.niva) }}
                </div>
              </div>
            </div>
      ...
      <v-flex class="grey lighten-4 px-3 pt-3 pb-5" xs8>
        <div v-if="$store.state.mallData.sammanfattning.length > 0" class="secondary--text body-1 pt-2">
