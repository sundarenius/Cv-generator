<template>
<div>
  <div v-if="$store.state.gettingData" class="mainBackground">
    <v-container fill-height style="min-height:100vh">
      <v-layout style="margin-top:-110px" justify-center wrap align-center>
        <v-flex pb-4 xs12 text-xs-center>
          <div style="font-weight:bold" :class="{'display-2' : $vuetify.breakpoint.smAndUp, 'title' : $vuetify.breakpoint.xsOnly}">
            Hämtar din data...
          </div>
        </v-flex>
        <v-flex text-xs-center xs10 md5>
          <template>
          <v-progress-linear style="border-radius:15px" height="15px" color="white" :indeterminate="true"></v-progress-linear>
        </template>
        </v-flex>
      </v-layout>
    </v-container>
  </div>
  <div v-if="!$store.state.gettingData" class="grey lighten-5">
    <div @click="$router.push('/')" v-show="$vuetify.breakpoint.mdAndUp && false" style="position:absolute;top:0px;left:-15px;border-radius:15px;transform:scale(0.8);cursor:pointer;z-index:100" class="ex15 black--text">
      CVFixarn.se
      <v-icon class="black--text" large style="margin-bottom:1px;margin-left:-12px">turned_in</v-icon>
    </div>
    <v-btn v-if="0" style="position:absolute;right:0;top:0;z-index:50" small flat class="green" @click="test">test</v-btn>
    <v-btn @click="saveChanges" round v-if="!watchDataChanges" style="position:fixed;top:5px;left:5px;z-index:130" class="success pointer">
      Spara ändringar
      <v-icon class="ml-2">save_alt</v-icon>
    </v-btn>
    <v-layout style="position:relative" pt-4 pb-5 row wrap>
      <v-flex style="position:relative" text-xs-center pt-1 pb-4 xs12 md6>
        ...
        <div v-if="0" style="position:absolute;top:-15px;right:20px;padding:1px 5px;border-radius:10px" class="secondary--text mainBackground">
        ...
        <div>
          <v-layout justify-center row>
            <v-flex xs8>
              ...
              <v-progress-linear height="8" style="border-radius:5px" :color="cvStrengthColorProgress" v-model="cvStrength"></v-progress-linear>
            </v-flex>
          </v-layout>
        </div>
        <v-layout justify-center row wrap px-5 py-5>
          ...
          <v-flex class="secondary--text" text-xs-left mt-2 xs12>
            ...
            <div :id="x.period" :key="x.id" v-for="(x, i) in $store.state.mallData.work" :class="{'my-3 px-2 py-2 v-card' : true, 'grey lighten-3' : i % 2 === 0, 'white' : i % 2 !== 0 }" style="position:relative">
          ...
          <v-flex class="secondary--text" text-xs-left mt-5 xs12>
            <div class="title bold">
             ...
             <div :key="x.id" v-for="(x, i) in $store.state.mallData.education" :class="{'my-3 px-2 py-2 v-card' : true, 'grey lighten-3' : i % 2 === 0, 'white' : i % 2 !== 0 }" style="position:relative">
              <div style="position:absolute;right:5px;top:5px">
                <v-tooltip bottom>
          ...
          <v-flex class="secondary--text" text-xs-left mt-5 mb-3 xs12>
            ...
            <v-layout row wrap>
              <v-flex style="position:relative" :class="{'px-2 py-2 my-1 v-card' : true, 'grey lighten-3' : i % 2 === 0, 'white' : i % 2 !== 0 }" :key="x.id" v-for="(x, i) in $store.state.mallData.languages" xs12>
          ...
          <v-flex class="secondary--text" text-xs-left mt-5 mb-3 xs12>
            ...
            <v-flex text-xs-center xs12>
              <v-layout row wrap>
                <v-flex style="position:relative" :class="{'px-2 py-2 my-1 v-card' : true, 'grey lighten-3' : i % 2 === 0, 'white' : i % 2 !== 0 }" :key="x.id" v-for="(x, i) in $store.state.mallData.referenser" xs12>
      ...
      <v-flex v-if="$vuetify.breakpoint.smAndUp" class="blue-grey" style="position:fixed;top:0;right:0;height:100vh;width:100vw;z-index:40" py-1 xs6>
        <div style="max-height:95vh;overflow:scroll;position:relative">
          <div v-if="0" style="position:fixed;top:10px;right:10px;border-radius:10px;z-index:50" class="secondary--text white px-1">
            <span class="caption bold">
              LIVE
            </span>
          </div>
          <mall1 v-if="$store.state.choosed === 1" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall2 v-if="$store.state.choosed === 2" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall3 v-if="$store.state.choosed === 3" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall4 v-if="$store.state.choosed === 4" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall5 v-if="$store.state.choosed === 5" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall6 v-if="$store.state.choosed === 6" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall7 v-if="$store.state.choosed === 7" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall8 v-if="$store.state.choosed === 8" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall9 v-if="$store.state.choosed === 9" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall10 v-if="$store.state.choosed === 10" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall11 v-if="$store.state.choosed === 11" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
          <mall12 v-if="$store.state.choosed === 12" style="transform:scale(0.95);position:sticky;top:0;right:10px" />
        ...
        <div style="position:absolute;bottom:15px;left:32%">
          <v-btn @click="downloadResume" round large class="success title px-5 bold">
            Ladda ner
            <v-icon v-if="downloadAble" class="ml-2">cloud_download</v-icon>
            <v-icon v-if="!downloadAble" class="ml-2">lock</v-icon>
          </v-btn>
        </div>
      </v-flex>
    ...
    <div style="position:absolute;z-index:-100;display:none">
      <img id="svgImg" :src="svgUrl" width="100%" alt="">
    </div>
...
export default {
  ...
  watch: {
    ...
    svgUrl: function () {
      const global = this
      if (this.svgUrl.length > 20) {
        setTimeout(function () {
          global.saveHtmlToPdfNext()
        }, 500)
      }
    }
  },
  computed: {
    downloadAble () {
      ...
      if (this.$store.state.mallData.payStatus === 0 && !this.$store.state.startDownload && !registered) {
        return false
      } else if ((this.$store.state.mallData.payStatus === 1 || this.$store.state.startDownload || registered)) {
        return true
      } else {
        return true
      }
